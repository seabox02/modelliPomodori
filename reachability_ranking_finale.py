...
OUTPUT_DIR   = "output_reachability2"
...
# Pesi del ranking geometrico (somma = 1.0)
W_AREA        = 0.45
W_CIRCULARITY = 0.35
W_CENTRALITY  = 0.20
...
# Pool di candidati geometrici da cui filtrare (deve essere > MAX_TARGETS)
GEOMETRIC_POOL = 8
...
# ── Stadio 2: Filtro di maturità cromatica ─────────────────────────
...
def select_targets(candidates, image_bgr):
    ...
    # Selezione finale con fallback
    selected = mature[:MAX_TARGETS]
    if len(selected) < MAX_TARGETS:
        remaining = MAX_TARGETS - len(selected)
        for d in immature[:remaining]:
            d["is_fallback"] = True
            selected.append(d)
...
def process_folder(model_path=MODEL_PATH, image_dir=IMAGE_DIR, output_dir=OUTPUT_DIR):
    ...
    for img_name in images:
        ...
        # Stadio 1: estrazione metriche geometriche
        candidates = []
        ...
        # Stadio 2: selezione con filtro maturità
        ...
        for rank, det in enumerate(top, 1):
            ...
            draw_rank_label(output, det["centroid"], rank, det["geo_score"], maturity, color)
